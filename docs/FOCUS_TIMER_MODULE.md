...
work category, classification, and optional note, then start and stop a server-backed
timer. On stop, Samay creates one or more ordinary draft timesheet entries for review.
...
- A normal same-day, non-overlapping timer produces start/end times. An overlapping,
  sub-minute, or cross-midnight capture is saved as duration-only draft data with a
  user warning, avoiding inaccurate or uneditable time windows.
...
| Active session | Operational state | `time_sessions` | Recovery, elapsed display, one-active-session invariant | Timer start/stop/discard APIs |
| Completed draft | Transaction | `timesheet_entries` / `timesheets` | Log Time, My Work, approvals, dashboards, reports | Created atomically by Stop, then existing edit/submit APIs |
| Elapsed seconds | Derived | `server_now - started_at` | Web pill, modal, PiP display | Recalculated; never incremented in storage |
...
- Body: `client_id`, `task_type`, `work_classification`, optional `description`,
  optional `source`
- Validates linked active masters and prevents a second running session.
...
One row per user, keyed by `user_id`. The row is overwritten when a new timer starts,
which is sufficient because completed work becomes an ordinary timesheet transaction.
The row retains the most recent stopped/discarded state until the next start.

### Firestore `time_sessions/{user_id}`

A deterministic document ID makes concurrent start attempts serializable. Stop and
draft creation run in one Firestore transaction.
...
- Firestore requires no destructive migration.
...
- Draft totals look wrong: run the timer-core tests and inspect timezone segmentation
  before changing reporting code.
...
| Concurrency | Two starts; two stops; start on a second device |
| Extreme | Timer over 24 hours; malformed timestamps; very long text |
| UI fit | 360x800, 768x1024, 1366x768, 1920x1080, compact PiP |
